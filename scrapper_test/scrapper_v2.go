		...
		if strings.Contains(table_id, "tabla1") {
			...
			e.ForEach("table tbody", func(_ int, el *colly.HTMLElement) {
				...
				ch.Find("tr").Each(func(clss int, tr *goquery.Selection) {
					tr.Find("td").Each(func(clss int, td *goquery.Selection) {
						fmt.Printf("%s", td.Find(".fecha").Text())
						fmt.Printf("%s", "Hola")
						td.Find(".fecha").Each(func(clss int, s *goquery.Selection) {
							fmt.Printf("%s", "Hola")
							//band, _ := s.Attr("class")
							//fmt.Println(band, s.Text())
						})
					})
				})
